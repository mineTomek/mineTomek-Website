...
export default function Item(props: { item: TimelineItem; index: number }) {
  return (
    <div className='flex flex-col gap-4 p-8 before:ml-1 md:relative md:w-1/2 md:before:absolute md:before:bottom-0 md:before:top-0 md:before:w-2 md:before:bg-zinc-400 md:before:first:[border-radius:0.375rem_0.375rem_0_0] md:before:last:[border-radius:0_0_0.375rem_0.375rem] md:odd:before:left-full md:even:left-1/2 md:even:before:-left-0'>
      <div className='flex justify-between text-xl'>
        <p className='bold'>{props.item.title}</p>
        <p className='text-zinc-500'>
          {props.item.date.toString().split('-').slice(0, 2).join('/')}
        </p>
      </div>
